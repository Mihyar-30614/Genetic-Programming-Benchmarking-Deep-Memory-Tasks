...
def generate_output(seq_length):
    result = np.zeros((seq_length*2 + 2, 2), dtype=np.float32)
    result[1:seq_length +1, 1] = 1
    result[seq_length +2:seq_length*2 +2, 0] = 1
    return result
...
def eval_genome(genome, config):
    ...
    total_error = 1.0 - (error / (length * num_tests))
    return total_error
...
def run():
    ...
    config_path = os.path.join(local_dir, 'CopyTaskSimplified_config')
